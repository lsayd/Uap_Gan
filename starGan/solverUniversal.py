#-*- coding = utf-8 -*-
#@Time : 2023/2/25 下午4:26
#@Author : lv
#@File : solverUniversal.py
#@Software : PyCharm
...
class SolverUniversal(object):
    ...
    def build_model(self):
        ...
        if self.dataset in ['CelebA', 'RaFD']:
            ...
            self.G = Generator(self.g_conv_dim, self.c_dim +
                               self.c2_dim+2, self.g_repeat_num)
            self.D = Discriminator(
    ...
    def restore_model(self, resume_iters):
        ...
        G_path=r"D:\lkq\lkq\stargan-master\stargan_celeba\models\200000-G.ckpt"
        # D_path = os.path.join(self.model_save_dir,
        #                       '{}-D.pt'.format(resume_iters))
        D_path = "stargan_celeba_256\models\D.pt"

        # self.G.load_state_dict(torch.load(G_path, map_location=lambda storage, loc: storage))
        self.load_model_weights(self.G, G_path)
        # self.D.load_state_dict(torch.load(
        #     "F:\ml\disrupting-deepfakes-master\stargan\stargan_celeba_256\models\200000-D.pt"))
    ...
    def build_and_restore_alt_model(self):
        ...
        if self.dataset in ['CelebA', 'RaFD']:
            ...
            # 2 for mask vector.
            self.G2 = Generator(self.g_conv_dim, self.c_dim +
                                self.c2_dim+2, self.g_repeat_num)
            self.D2 = Discriminator(
                self.image_size, self.d_conv_dim, self.c_dim+self.c2_dim, self.d_repeat_num)

        self.g_optimizer2 = torch.optim.Adam(
            self.G2.parameters(), self.g_lr, [self.beta1, self.beta2])
        self.d_optimizer2 = torch.optim.Adam(
            self.D2.parameters(), self.d_lr, [self.beta1, self.beta2])
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            d_loss_cls = self.classification_loss(
                out_cls, label_org, self.dataset)
            ...
            x_hat = (alpha * x_real.data + (1 - alpha) *
                     x_fake.data).requires_grad_(True)  # No Attack
            ...
            if (i+1) % self.n_critic == 0:
                ...
                g_loss = g_loss_fake + self.lambda_rec * \
                    g_loss_rec + self.lambda_cls * g_loss_cls
                self.reset_grad()
                g_loss.backward()
            ...
            if (i+1) % self.model_save_step == 0:
                ...
                D_path = os.path.join(
                    self.model_save_dir, '{}-D.pt'.format(i+1))
    ...
    def train_adv_gen(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            pgd_attack = attacks.LinfPGDAttack(
                model=self.G, device=self.device, feat=None)
            # =================================================================================== #
            #                             2. Train the discriminator                              #
            # =================================================================================== #
            ...
            if (i+1) % self.n_critic == 0:
                ...
                pgd_attack = attacks.LinfPGDAttack(
                    model=self.G, device=self.device, feat=None)
                x_real_adv = attacks.perturb_batch(
                ...
                # Target-to-original domain.
                x_fake_adv = attacks.perturb_batch(
    ...
    def train_adv_both(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            pgd_attack = attacks.LinfPGDAttack(
                model=self.G, device=self.device, feat=None)
            x_real_adv = attacks.perturb_batch(
                x_real, black, c_trg, self.G, pgd_attack)    # Adversarial training
            ...
            x_fake, _ = self.G(x_real_adv, c_trg)   # Attack
            x_fake_adv = attacks.perturb_batch(
                x_fake, black, c_org, self.G, pgd_attack)    # Adversarial training
            out_src, out_cls = self.D(x_fake_adv.detach())  # Attack
            d_loss_fake = torch.mean(out_src)

            # Compute loss for gradient penalty.
            alpha = torch.rand(x_real.size(0), 1, 1, 1).to(self.device)
            x_hat = (alpha * x_real_adv.data + (1 - alpha) *
                     x_fake_adv.data).requires_grad_(True)  # Attack
            out_src, _ = self.D(x_hat)
            d_loss_gp = self.gradient_penalty(out_src, x_hat)
    ...
    def train_multi(self):
        ...
        zero_celeba = torch.zeros(x_fixed.size(0), self.c_dim).to(self.device)
        # Zero vector for RaFD.
        zero_rafd = torch.zeros(x_fixed.size(0), self.c2_dim).to(self.device)
        # Mask vector: [1, 0].
        mask_celeba = self.label2onehot(
            torch.zeros(x_fixed.size(0)), 2).to(self.device)
        # Mask vector: [0, 1].
        mask_rafd = self.label2onehot(
            torch.ones(x_fixed.size(0)), 2).to(self.device)
        ...
        for i in range(start_iters, self.num_iters):
            for dataset in ['CelebA', 'RaFD']:
                ...
                out_cls = out_cls[:,
                                  :self.c_dim] if dataset == 'CelebA' else out_cls[:, self.c_dim:]
                d_loss_real = - torch.mean(out_src)
                d_loss_cls = self.classification_loss(
        ...
        for i, (x_real, c_org) in enumerate(data_loader):
            ...
            for idx, c_trg in enumerate(c_trg_list):
                ...
                x_adv, perturb = pgd_attack.perturb(
                    x_real, gen_noattack, c_trg)                          # Vanilla attack
    ...
    def get_universal_perturbation(self):
        ...
        model_G=self.G
        model_D=self.D
        universal_pert =universal(model_G=model_G,model_D=model_D, device=self.device)
    ...
    def test_universal_OnImages(self):
        ...
        for i, (x_real, c_org) in enumerate(data_loader):
            ...
            # Save the translated images.
            x_concat = torch.cat(x_fake_list, dim=3)
            save_dir="D:\lkq\disrupting-deepfakes-master\stargan\\results_universal"
            result_path = os.path.join(
